...
func TestGetLKEClusterPool_ready(t *testing.T) {
	client, lkeCluster, pool, teardown, err := setupLKEClusterPool(t, "fixtures/TestGetLKEClusterPool_ready", withReadyPool)
	if err != nil {
		t.Error(err)
	}
	defer teardown()
...
type clusterPoolMods func(*linodego.Client, *linodego.LKECluster, *linodego.LKEClusterPool) error

func withReadyPool(client *linodego.Client, cluster *linodego.LKECluster, pool *linodego.LKEClusterPool) (err error) {
	cluster, err = client.WaitForLKEClusterReady(context.Background(), cluster.ID, 300)
	if err != nil {
		return err
	}
	pool, err = client.WaitForLKEClusterPoolReady(context.Background(), cluster.ID, pool.ID, 300)
	return err
}

func setupLKEClusterPool(t *testing.T, fixturesYaml string, modifications ...clusterPoolMods) (*linodego.Client, *linodego.LKECluster, *linodego.LKEClusterPool, func(), error) {
	t.Helper()
	...
	for _, mod := range modifications {
		if err := mod(client, lkeCluster, pool); err != nil {
			t.Errorf("Error modifying setupLKEClusterPool resources, got error %v", err)
		}
	}
